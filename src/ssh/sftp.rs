...
pub(super) async fn upload_file(
	...
	// It does NOT expand `~/` like a shell would. Therefore, we strip `~/` so SFTP
	// looks in the home directory instead of looking for a literal `~` folder.
	let sftp_path = remote_path.strip_prefix("~/").unwrap_or(remote_path);
