...
///
/// Accepts string values like `30d`, `12h`, `45m`, `60s`, or a string containing
/// only digits, which defaults to minutes (e.g. `"30"` means 30 minutes).
/// Parsing is handled by [`humantime`] with an added convenience for bare
/// numeric strings.
...
impl Default for HumanDuration {
	fn default() -> Self {
		// Default to 30 days.
		Self(Duration::from_hours(30 * 24))
	}
}
...
impl FromStr for HumanDuration {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		if let Ok(minutes) = s.parse::<u64>() {
			return Ok(Self(Duration::from_mins(minutes)));
		}
...
mod tests {
	...
	fn parse_days() {
		...
		assert_eq!(d.as_duration(), Duration::from_hours(30 * 24));
	}

	#[test]
	fn parse_hours() {
		let d: HumanDuration = "12h".parse().unwrap();
		assert_eq!(d.as_duration(), Duration::from_hours(12));
	}

	#[test]
	fn parse_minutes() {
		let d: HumanDuration = "45m".parse().unwrap();
		assert_eq!(d.as_duration(), Duration::from_mins(45));
	}

	#[test]
	fn parse_seconds() {
		let d: HumanDuration = "60s".parse().unwrap();
		assert_eq!(d.as_duration(), Duration::from_mins(1));
	}

	#[test]
	fn parse_bare_number_defaults_to_minutes() {
		let d: HumanDuration = "30".parse().unwrap();
		assert_eq!(d.as_duration(), Duration::from_mins(30));
	}
