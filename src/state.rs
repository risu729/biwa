...
/// Returns the default biwa state directory based on platform conventions.
#[must_use]
...
pub fn load_state(state_dir: &Path) -> Result<State> {
	let path = connections_file_path(state_dir);
	if !path.exists() {
...
pub fn save_state(state_dir: &Path, state: &State) -> Result<()> {
	...
	let tmp_path = state_dir.join(format!("{CONNECTIONS_FILE}.{}.tmp", process::id()));
...
#[must_use]
pub fn read_daemon_pid(state_dir: &Path) -> Option<i32> {
	let path = pid_file_path(state_dir);
...
pub fn kill_daemon(state_dir: &Path) {
	let Some(pid) = read_daemon_pid(state_dir) else {
		return;
	};
...
mod tests {
	...
	fn record_and_load_connection() {
		...
		record_connection(dir.path(), "host", "user", 22, "/remote/dir").unwrap();
	...
	fn stale_connections_filters_by_age() {
		...
		let stale = stale_connections(&state, Duration::from_hours(30 * 24));
	...
	fn remove_connections_from_state() {
		...
		record_connection(dir.path(), "host", "user", 22, "/dir1").unwrap();
		record_connection(dir.path(), "host", "user", 22, "/dir2").unwrap();
		remove_connections_for_target(dir.path(), "host", "user", 22, &["/dir1"]).unwrap();
		let state = load_state(dir.path()).unwrap();
		assert_eq!(state.connections.len(), 1);
	...
	#[test]
	#[serial]
	fn kill_daemon_without_a_pid_file_is_a_no_op() {
		let dir = tempfile::tempdir().unwrap();
