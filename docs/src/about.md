...
biwa is the spiritual successor to **[`cserun`](https://cserun.bojin.co/)**. We are deeply grateful to [Bojin](https://github.com/xxxbrian), the author of `cserun`, for pioneering this approach and providing the tool that inspired biwa.
...
_The filesystem mount approach_
...
_The graphical desktop approach_
...
- **Smart Sync (rsync)**: Unlike `cserun` which clones the directory from scratch every time (wasting disk space and time), **biwa uses `rsync`** to synchronize _only changed files_.
