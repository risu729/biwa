...
Interactive support is still partial, though: biwa forwards local `Ctrl-C` interrupts to the remote command, but it does not yet provide full terminal emulation such as raw mode or window resize propagation. For complex full-screen or highly interactive programs, use `ssh` directly for the most reliable behavior.
:::
