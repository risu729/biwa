...
#[test]
fn e2e_sync_pull_overwrite_downloads_remote_file() -> Result<()> {
	let dir = tempfile::tempdir()?;
	let remote_proj_dir = common::get_remote_project_dir(dir.path())?;
	...
	let output = biwa_cmd_tilde(&["sync", "--pull", "--overwrite"], dir.path())
		.stdout_capture()
		.stderr_capture()
		.unchecked()
		.run()?;
...
#[test]
fn e2e_sync_pull_updates_generated_file_by_default() -> Result<()> {
	let dir = tempfile::tempdir()?;
	fs::write(
...
#[test]
fn e2e_sync_pull_preserves_non_generated_file_by_default() -> Result<()> {
	let dir = tempfile::tempdir()?;
	...
	let stderr = String::from_utf8_lossy(&output.stderr);
	assert!(output.status.success(), "stderr: {stderr}");
	assert!(stderr.contains("0 downloaded"), "stderr: {stderr}");
	assert_eq!(fs::read_to_string(dir.path().join("source.md"))?, "local");

	Ok(())
}

#[test]
fn e2e_sync_pull_overwrite_deletes_local_file_missing_remotely() -> Result<()> {
	let dir = tempfile::tempdir()?;
...
#[test]
fn e2e_sync_pull_overwrite_creates_and_removes_empty_dirs() -> Result<()> {
	let dir = tempfile::tempdir()?;
...
fn e2e_sync_pull_respects_include_scope() -> Result<()> {
	...
	let output = biwa_cmd_tilde(
		&["sync", "--pull", "--overwrite", "--include", "kept.txt"],
		dir.path(),
	)
	.stdout_capture()
	.stderr_capture()
